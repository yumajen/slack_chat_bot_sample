...
function judgeAndReply_(text) {
    const systemText = [
        ...
        "",
        "【返答ルール（OKの場合）】",
        "- 共感 or 乗っかる一言 + 質問は最大1つまで（なくてもよい）",
        "- 質問が来た場合はそれに答えるのもOKだが、必ずしも質問返しは必要ない",
        "- 日本語 / 80〜180文字 / 前置き不要",
        "- 個人攻撃禁止",
        "- NGルールに触れない内容であれば断定表現・具体名OK",
        "- 伏せ字（⚪︎⚪︎・〇〇・XX等）一切禁止",
        "",
    ...
    setProp_(PROP.DEBUG_LAST_NG_RESULT, JSON.stringify({ input: text, raw, allowed: result.allowed }));
